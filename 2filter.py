...
bar2 = alt.Chart(df2, title='Tooltip: happiness score for each region').mark_bar(opacity=0.3,).encode(
    ...
    alt.Y('mean(Happiness_score)',scale=alt.Scale(zero=False),title='Happiness score'), tooltip= 'mean(Happiness_score)',
        color=alt.Color('Region'),
      opacity= "online:O").interactive().properties(height=350,width=430)
